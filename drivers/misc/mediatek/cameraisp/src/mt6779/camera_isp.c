	...
	struct file *pFile)
{
	...
	#ifdef VENDOR_EDIT
	/*Yijun.Tan@Camera add for resolve monkey open camera fail 20190403*/
...
RESET:
	...
	LOG_DBG("%s_TG_ST(%d)_SW_ST(0x%x), sec %ld\n", moduleName, regTGSt,
...
static void SMI_INFO_DUMP(enum ISP_IRQ_TYPE_ENUM irq_module)
{
	...
	case ISP_IRQ_TYPE_INT_CAM_C_ST:
		...
		#ifdef VENDOR_EDIT
		/*Yijun.Tan@Camera add for resolve monkey ISP pass1 deque fail 20190403*/
		if (g_ISPIntStatus_SMI[irq_module].ispIntErr &
		    TG_ERR_ST) {
